"""Parser implementation for KFintech mutual fund report files."""
...
KFINTECH_ALIASES = {
    "pan": ["pan_no", "pan", "pannumber", "panno"],
    ...
    "fatca_status": ["fatca", "fatcastatus"],
    "investor_status": ["status", "investorstatus"],
...
class KFintechParser(BaseParser):
    """Parser class for loading and validating KFintech reports."""

    def __init__(self, header_mapper: HeaderMapper | None = None) -> None:
        self.header_mapper = header_mapper or HeaderMapper(aliases=KFINTECH_ALIASES)
        self.source = RecordSource.KFINTECH

    def parse(self, file_path: Path) -> list[ReportRecord]:
        """Parses KFintech file and returns a flat list of successfully validated ReportRecord objects."""
        result = self.parse_report(file_path)
        return result.records

    def parse_report(self, file_path: Path, sheet_name: str | None = None) -> ReportParseResult:
        """
        Parses a KFintech report (CSV or Excel) and returns a detailed ReportParseResult.
        ...
        if file_path.stat().st_size == 0:
            logger.error(f"KFintech parser failed: file {file_path.name} is empty.")
            raise ParserError(f"KFintech report file is empty: {file_path.name}")
        ...
        try:
            if suffix == ".csv":
                ...
                logger.error(f"Unsupported KFintech report extension: {suffix}")
        ...
        logger.info(
            f"KFintech Parsing Complete: {file_path.name}. Total: {total_rows}, "
