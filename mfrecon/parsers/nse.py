"""Parser implementation for NSE mutual fund report files."""
...
NSE_ALIASES = {
    "pan": ["nse_pan", "pan", "pannumber", "panno", "primary holder pan", "primaryholderpan"],
    "investor_name": ["name", "investorname", "clientname", "nse_client_name", "investor name"],
    "mobile": ["mobile", "mobilenumber", "contactnumber", "phone", "indian mobile no.", "indian mobile no", "indianmobileno"],
    "email": ["email", "emailaddress"],
    "total_amount": ["amount", "totalamount", "balance"],
    "folio_number": ["folio_number", "folio", "folionumber", "foliono", "client_code", "clientcode", "code", "client_id", "clientid", "nse_client_code"],
    "kyc_status": ["kyc", "kycstatus", "kyc status", "primary holder kyc type"],
    "tax_status": ["tax_status", "taxstatus", "taxability", "taxcategory", "tax_status_desc", "tax status"],
    "date_of_birth": ["date_of_birth", "dob", "birth_date", "birthdate", "primary holder dob/incorporation", "primary holder dob"],
    "bank_name": ["bank_name", "bankname", "bank", "bank name", "bank name 1"],
    "bank_account": ["bank_account", "bank_account_number", "account_no", "account_number", "bank_ac", "ac_no", "ac_number", "account no", "account no 1"],
    "ifsc": ["ifsc", "ifsc_code", "ifsccode", "ifsc code", "ifsc code 1"],
    "registered_address": ["registered_address", "address", "residence_address", "registered address", "address 1"],
    "mode_of_holding": ["mode_of_holding", "holding_mode", "mode", "holding_type", "holding nature", "holdingnature"],
    "nominee_1": ["nominee", "nominee_1", "nominee_name", "nominee1", "nominee 1 name"],
    "nominee_relation": ["nominee_relation", "nominee_relation_percentage", "relation", "nominee_relationship", "nominee 1 relationship"],
    "distributor_arn": ["distributor_arn", "arn", "distributor", "broker_code", "broker_arn"]
}

class NSEParser(BaseParser):
    """Parser class for loading and validating NSE reports."""

    def __init__(self, header_mapper: HeaderMapper | None = None) -> None:
        self.header_mapper = header_mapper or HeaderMapper(aliases=NSE_ALIASES)
        self.source = RecordSource.NSE

    def parse(self, file_path: Path) -> list[ReportRecord]:
        """Parses NSE file and returns a flat list of successfully validated ReportRecord objects."""
        result = self.parse_report(file_path)
        return result.records

    def parse_report(self, file_path: Path, sheet_name: str | None = None) -> ReportParseResult:
        """
        Parses an NSE report (CSV or Excel) and returns a detailed ReportParseResult.
        ...
        if file_path.stat().st_size == 0:
            logger.error(f"NSE parser failed: file {file_path.name} is empty.")
            raise ParserError(f"NSE report file is empty: {file_path.name}")
        ...
        try:
            if suffix == ".csv":
                ...
                logger.error(f"Unsupported NSE report extension: {suffix}")
        ...
        logger.info(
            f"NSE Parsing Complete: {file_path.name}. Total: {total_rows}, "
