"""Parser implementation for BSE mutual fund report files."""
...
BSE_ALIASES = {
    "pan": ["bse_pan", "pan", "pannumber", "panno", "primary holder pan", "primaryholderpan"],
    "investor_name": ["name", "investorname", "clientname", "bse_client_name", "investor name"],
    ...
    "folio_number": ["folio_number", "folio", "folionumber", "foliono", "client_code", "clientcode", "code", "client_id", "clientid", "bse_client_code"],
...
class BSEParser(BaseParser):
    """Parser class for loading and validating BSE reports."""

    def __init__(self, header_mapper: HeaderMapper | None = None) -> None:
        self.header_mapper = header_mapper or HeaderMapper(aliases=BSE_ALIASES)
        self.source = RecordSource.BSE

    def parse(self, file_path: Path) -> list[ReportRecord]:
        """Parses BSE file and returns a flat list of successfully validated ReportRecord objects."""
        result = self.parse_report(file_path)
        return result.records

    def parse_report(self, file_path: Path, sheet_name: str | None = None) -> ReportParseResult:
        """
        Parses a BSE report (CSV or Excel) and returns a detailed ReportParseResult.
        ...
        if file_path.stat().st_size == 0:
            logger.error(f"BSE parser failed: file {file_path.name} is empty.")
            raise ParserError(f"BSE report file is empty: {file_path.name}")
        ...
        try:
            if suffix == ".csv":
                ...
                logger.error(f"Unsupported BSE report extension: {suffix}")
        ...
        logger.info(
            f"BSE Parsing Complete: {file_path.name}. Total: {total_rows}, "
